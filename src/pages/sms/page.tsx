  ...
  return (
    <AdminLayout title="SMS Campaigns" subtitle="Bulk Messaging · Payment Reminders · Delivery Updates">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-4 mb-5">
        {[
          { label: 'Total Sent', value: smsStats.totalSent.toLocaleString(), icon: 'ri-send-plane-line', color: '#EC0118' },
          ...
          <div className="divide-y divide-slate-100">
            {smsCampaigns.map((campaign) => (
              <div key={campaign.id} className="p-4 flex items-start gap-4 hover:bg-slate-50/50 transition-colors">
                <div className="w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0" style={{ background: campaign.type === 'Promotional' ? '#F59E0B15' : 'rgba(236,1,24,0.08)' }}>
                  <i className={`${campaign.type === 'Promotional' ? 'ri-megaphone-line' : 'ri-mail-send-line'} text-lg`} style={{ color: campaign.type === 'Promotional' ? '#F59E0B' : '#EC0118' }} />
                </div>
          ...
          {smsSegments.map((segment) => (
            <div key={segment.id} className="bg-white rounded-2xl p-4 border border-slate-100">
              <div className="flex items-center justify-between mb-3">
                ...
                <span className="text-lg font-bold" style={{ color: '#EC0118' }}>{segment.count.toLocaleString()}</span>
              </div>
              <p className="text-xs text-slate-500 mb-4">{segment.description}</p>
              <div className="flex items-center gap-2">
                <div className="flex-1 h-1.5 rounded-full bg-slate-100 overflow-hidden">
                  <div className="h-full rounded-full" style={{ width: `${(segment.count / 2840) * 100}%`, background: '#EC0118' }} />
                </div>
              ...
              <button
                ...
                style={{ borderColor: '#EC0118', color: '#EC0118' }}
              >
                Send to Segment
              </button>
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
          <div className="bg-white rounded-2xl p-5 border border-slate-100">
            ...
            <div className="space-y-4">
              {[
                { label: 'Sent', value: smsStats.totalSent, max: smsStats.totalSent, color: '#EC0118' },
          ...
          <div className="bg-white rounded-2xl p-5 border border-slate-100">
            ...
            <div className="flex items-center justify-center py-8">
              <div className="relative w-40 h-40">
                <svg viewBox="0 0 100 100" className="w-full h-full -rotate-90">
                  <circle cx="50" cy="50" r="40" fill="none" stroke="rgba(236,1,24,0.08)" strokeWidth="12" />
                  <circle
                    cx="50" cy="50" r="40" fill="none" stroke="#EC0118"
                    strokeWidth="12"
            ...
            <div className="flex items-center justify-center gap-6 text-xs">
              <div className="flex items-center gap-2">
                <span className="w-3 h-3 rounded-full" style={{ background: '#EC0118' }} />
