...
export default function CalendarPage() {
  ...
  const stats = [
    { label: "Today's Appts", value: String(todayEvents.length), icon: 'ri-calendar-check-line', color: '#EC0118' },
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <div className="lg:col-span-2 bg-white rounded-2xl border border-slate-100 overflow-hidden">
          ...
          <div className="grid grid-cols-7 border-b border-slate-100">
            {weekDays.map((day, i) => (
              <div key={day} className={`p-3 text-center ${i === currentDayIndex ? 'bg-[rgba(236,1,24,0.05)]' : ''}`}>
                <p className="text-[10px] text-slate-400 uppercase">{day}</p>
                <p className={`text-sm font-semibold ${i === currentDayIndex ? 'text-[#EC0118]' : 'text-slate-700'}`}>{currentWeekDates[i]}</p>
              </div>
            ))}
          </div>
          ...
          <div className="grid grid-cols-7 divide-x divide-slate-100 min-h-[400px]">
            {weekDays.map((_, dayIndex) => {
              ...
              return (
                <div key={dayIndex} className={`p-2 space-y-2 ${dayIndex === currentDayIndex ? 'bg-[rgba(236,1,24,0.03)]' : ''}`}>
                  {dayEvents.map((event) => (
                    <button
                      key={event.id}
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="flex gap-2 pt-2">
                        <button
                          ...
                          className="flex-1 py-2 rounded-lg text-xs font-semibold text-white cursor-pointer whitespace-nowrap" style={{ background: '#EC0118' }}>
                          Confirm
                        </button>
                        <button
