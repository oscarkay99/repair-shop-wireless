...
const tabs = [
  ...
  { id: 'repairs',     label: 'Repairs',     icon: 'ri-tools-line'           },
  ...
  return (
    <AdminLayout title="Analytics" subtitle="Repair performance, revenue and customer insights">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 mb-5">
        <div className="flex items-center gap-1 bg-white border border-slate-100 rounded-2xl p-1.5 flex-wrap">
          {tabs.map((tab) => (
            <button
              key={tab.id}
      ...
      {activeTab === 'repairs' && (
        <div className="space-y-5">
          ...
          <div className="bg-white rounded-2xl p-6 border border-slate-100">
            <h3 className="text-sm font-bold text-slate-800 mb-5">Repairs by Device Type</h3>
              ...
              {(() => {
                ...
                return Object.entries(counts).sort((a,b)=>b[1]-a[1]).map(([type, count], i) => (
                  <div key={type} className="bg-slate-50 rounded-2xl p-4 text-center">
                    ...
                    <div className="text-xs font-semibold text-slate-600 mt-1">{type}</div>
                    <div className="text-[10px] text-slate-400">{Math.round(count / repairs.length * 100)}% of jobs</div>
                  </div>
                ));
              })()}
            </div>
          ...
          {/* Common issues */}
          <div className="bg-white rounded-2xl p-6 border border-slate-100">
            <h3 className="text-sm font-bold text-slate-800 mb-4">Most Common Issues</h3>
            <div className="space-y-2">
              {(() => {
                ...
                return Object.entries(issues).sort((a,b)=>b[1]-a[1]).slice(0,6).map(([issue, count]) => (
                  <div key={issue} className="flex items-center justify-between py-2 border-b border-slate-50 last:border-0">
        ...
        <div className="space-y-5">
          ...
          <div className="bg-white rounded-2xl border border-slate-100 overflow-hidden">
            <div className="px-5 py-4 border-b border-slate-100">
              <h3 className="text-sm font-bold text-slate-800">Revenue from Repairs — Monthly</h3>
            </div>
              ...
              <table className="w-full">
                ...
                <tbody>
                  {[
                    ...
                    <tr key={d.month} className={`border-b border-slate-50 hover:bg-slate-50/60 ${i%2===0?'':'bg-slate-50/20'}`}>
                      <td className="px-4 py-3 text-xs font-semibold text-slate-800">{d.month} 2026</td>
                      <td className="px-4 py-3 text-xs text-slate-700">{d.jobs}</td>
                      <td className="px-4 py-3 text-xs font-bold text-slate-900">GHS {d.revenue.toLocaleString()}</td>
                      <td className="px-4 py-3 text-xs text-slate-700">GHS {Math.round(d.revenue/d.jobs).toLocaleString()}</td>
                      <td className="px-4 py-3 text-xs text-slate-700">GHS {d.parts.toLocaleString()}</td>
      ...
      {activeTab === 'customers' && (
        <div className="space-y-5">
          <CustomerSegments />
          <div className="bg-white rounded-2xl p-6 border border-slate-100">
            <h3 className="text-sm font-bold text-slate-800 mb-5">Customer Retention</h3>
      ...
      {activeTab === 'operations' && (
        <div className="space-y-5">
          <RepairAnalytics />
          <div className="bg-white rounded-2xl p-6 border border-slate-100">
            <h3 className="text-sm font-bold text-slate-800 mb-5">Technician Performance</h3>
