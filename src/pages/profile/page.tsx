...
export default function ProfilePage() {
  ...
  const { permission: pushPermission, subscribing: pushSubscribing, subscribe: enablePush, isSupported: pushSupported } = usePushNotifications();
    ...
    <AdminLayout title="My Profile" subtitle="Manage your account, security and preferences">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          {/* NOTIFICATIONS — a persistent path to the browser permission
              prompt. The floating PushNotificationPrompt only ever offers
              this once per device (a "Not now" or a denial hides it for
              good, and a browser can't be asked to re-prompt from JS after
              a denial) — this is where anyone who skipped or declined it
              can come back and try again. */}
          {activeTab === 'profile' && (
            <div className="bg-[hsl(var(--card))] rounded-2xl border border-[hsl(var(--border))] p-6">
              <h3 className="text-sm font-bold text-[hsl(var(--foreground))] mb-1">Notifications</h3>
              <p className="text-xs text-[hsl(var(--muted-foreground))] mb-4">
                Get notified here even when Wireless isn't open — new tickets assigned to you, status changes, and more.
              </p>

              {!pushSupported ? (
                <div className="flex items-start gap-3 p-3 rounded-xl bg-[hsl(var(--muted))]">
                  <i className="ri-information-line text-[hsl(var(--muted-foreground))] text-sm mt-0.5" />
                  <p className="text-xs text-[hsl(var(--muted-foreground))] leading-relaxed">
                    Not available in this browser. On an iPhone, first add Wireless to your Home Screen (Share → Add to Home Screen) and open it from there — Safari alone can't receive notifications.
                  </p>
                </div>
              ) : pushPermission === 'granted' ? (
                <div className="flex items-center gap-2 p-3 rounded-xl bg-green-50 border border-green-100">
                  <i className="ri-check-double-line text-green-500 text-sm" />
                  <p className="text-xs text-green-600 font-medium">Notifications are on for this device</p>
                </div>
              ) : pushPermission === 'denied' ? (
                <div className="flex items-start gap-3 p-3 rounded-xl bg-amber-50 border border-amber-100">
                  <i className="ri-error-warning-line text-amber-500 text-sm mt-0.5" />
                  <p className="text-xs text-amber-700 leading-relaxed">
                    Notifications are blocked for Wireless in this browser. They can only be turned back on from your browser or phone's own site settings, not from here — look for Wireless under Settings → Notifications (or the site's lock/info icon in the address bar) and allow notifications there.
                  </p>
                </div>
              ) : (
                <button
