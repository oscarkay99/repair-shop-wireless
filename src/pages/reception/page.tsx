...
import {
  ...
  FileText, ShoppingCart, Package, HardHat,
} from 'lucide-react';
...
const NAV_TABS = [
  { key: 'tickets',     label: 'Tickets',     to: '/tickets',     icon: ClipboardList },
  { key: 'invoices',    label: 'Invoices',    to: '/invoices',    icon: FileText },
  { key: 'sales',       label: 'Sales',       to: '/sales',       icon: ShoppingCart },
  { key: 'inventory',   label: 'Inventory',   to: '/inventory',   icon: Package },
  { key: 'technicians', label: 'Technicians', to: '/technicians', icon: HardHat },
  { key: 'birthdays',   label: 'Birthdays',   to: null,           icon: Cake },
] as const;
...
export default function ReceptionPortalPage() {
  ...
  return (
    <div className="h-screen flex flex-col" style={{ background: 'hsl(var(--background))' }}>
      {/* Header */}
      <header className="flex items-center justify-between px-4 sm:px-6 h-14 border-b flex-shrink-0" style={{ borderColor: 'hsl(var(--border))' }}>
        <div className="flex items-center gap-3">
          <div
            className="w-8 h-8 rounded-lg flex-shrink-0 overflow-hidden flex items-center justify-center"
            style={{ background: 'rgb(15,15,15)', boxShadow: '0 0 10px 2px rgba(255,0,0,0.7), 0 0 24px 5px rgba(255,0,0,0.4)' }}
          >
            <img src="/wireless-mark.png" alt="WIRELESS" className="w-full h-full object-cover" />
          </div>
          <div>
            <p className="text-xs font-black tracking-[0.12em] uppercase leading-none" style={{ color: 'hsl(var(--foreground))' }}>WIRELESS</p>
            <p className="text-[9px] tracking-widest uppercase leading-none mt-0.5" style={{ color: 'hsl(var(--muted-foreground))' }}>Reception</p>
          </div>
        </div>
      ...
      {/* Tabs */}
      <nav className="flex items-center gap-1 px-4 sm:px-6 border-b flex-shrink-0 overflow-x-auto" style={{ borderColor: 'hsl(var(--border))' }}>
        {NAV_TABS.map(tab => {
          ...
          const badge = tab.key === 'invoices' ? invoicesNeedingAttention
            ...
            : 0;
          const Icon = tab.icon;
          const tabClass = "flex items-center gap-1.5 px-4 py-2.5 text-xs font-semibold whitespace-nowrap border-b-2 -mb-px transition-colors cursor-pointer";
          const tabStyle = {
