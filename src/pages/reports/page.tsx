    ...
    <AdminLayout title="Reports & Export" subtitle="Generate · Download · Share business reports">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-5">
        {kpiData.slice(0, 4).map(kpi => (
          <div key={kpi.label} className="bg-[hsl(var(--card))] rounded-2xl p-4 border border-[hsl(var(--border))]">
      ...
      {/* Tabs */}
      <div className="flex border border-[hsl(var(--border))] rounded-xl p-1 bg-[hsl(var(--card))] mb-5 w-fit">
      ...
      {activeTab === 'overview' && (
        <div className="space-y-5">
          ...
          <div className="bg-[hsl(var(--card))] rounded-2xl border border-[hsl(var(--border))] p-5">
            <h3 className="text-sm font-bold text-[hsl(var(--foreground))] mb-4">Key Metrics</h3>
          ...
          {/* Quick Generate */}
          <div className="bg-[hsl(var(--card))] rounded-2xl border border-[hsl(var(--border))] p-5">
            <h3 className="text-sm font-bold text-[hsl(var(--foreground))] mb-4">Quick Generate</h3>
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
              {reportTypes.map(rt => (
                <button
                  key={rt.id}
                  onClick={() => { setSelectedReport(rt.id); setActiveTab('generate'); }}
                  className="flex flex-col items-start gap-2 p-4 rounded-2xl border border-[hsl(var(--border))] hover:border-[hsl(var(--border))] hover:bg-[hsl(var(--muted))] transition-all cursor-pointer text-left"
                >
                  ...
                  <div>
                    <p className="text-xs font-semibold text-[hsl(var(--foreground))]">{rt.name}</p>
                    <p className="text-[10px] text-[hsl(var(--muted-foreground))] mt-0.5 line-clamp-2">{rt.desc}</p>
                  </div>
                </button>
              ))}
            </div>
      ...
      {activeTab === 'generate' && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
          ...
          <div className="bg-[hsl(var(--card))] rounded-2xl border border-[hsl(var(--border))] p-4">
            <h3 className="text-sm font-bold text-[hsl(var(--foreground))] mb-3">Report Type</h3>
          ...
          <div className="lg:col-span-2 space-y-4">
            <div className="bg-[hsl(var(--card))] rounded-2xl border border-[hsl(var(--border))] p-5">
              <h3 className="text-sm font-bold text-[hsl(var(--foreground))] mb-4">Report Configuration</h3>
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="text-xs text-[hsl(var(--muted-foreground))] mb-1 block">Date From</label>
                    <input type="date" value={dateFrom} onChange={(e) => setDateFrom(e.target.value)} className="w-full px-4 py-2.5 rounded-xl bg-[hsl(var(--muted))] text-sm text-[hsl(var(--foreground))] focus:outline-none focus:ring-2 focus:ring-[hsl(var(--border))]" />
                  </div>
                  <div>
                    <label className="text-xs text-[hsl(var(--muted-foreground))] mb-1 block">Date To</label>
                    <input type="date" value={dateTo} onChange={(e) => setDateTo(e.target.value)} className="w-full px-4 py-2.5 rounded-xl bg-[hsl(var(--muted))] text-sm text-[hsl(var(--foreground))] focus:outline-none focus:ring-2 focus:ring-[hsl(var(--border))]" />
                  </div>
                </div>
                <div>
                  <label className="text-xs text-[hsl(var(--muted-foreground))] mb-2 block">Quick Date Range</label>
                  <div className="flex gap-2 flex-wrap">
                    {['Today', 'This Week', 'This Month', 'Last Month', 'Q1 2026', 'YTD'].map(range => (
                      <button key={range} className="px-3 py-1.5 rounded-full text-xs font-semibold bg-[hsl(var(--muted))] text-[hsl(var(--muted-foreground))] hover:bg-[hsl(var(--muted))] cursor-pointer whitespace-nowrap border border-[hsl(var(--border))]">
                        {range}
                      </button>
                    ))}
                  </div>
                ...
                <div>
                  <label className="text-xs text-[hsl(var(--muted-foreground))] mb-2 block">Export Format</label>
                ...
                <div>
                  <label className="text-xs text-[hsl(var(--muted-foreground))] mb-2 block">Include Sections</label>
                  <div className="grid grid-cols-2 gap-2">
                    {['Summary', 'Charts', 'Detailed Tables', 'Comparisons', 'Recommendations', 'Raw Data'].map(section => (
                      <label key={section} className="flex items-center gap-2 p-2 rounded-lg hover:bg-[hsl(var(--muted))] cursor-pointer">
                        <input type="checkbox" defaultChecked className="rounded" />
                        <span className="text-xs text-[hsl(var(--foreground))]">{section}</span>
                      </label>
                    ))}
                  </div>
                </div>
            ...
            {generated ? (
              <div className="bg-[hsl(var(--card))] rounded-2xl border border-green-200 p-5">
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <div>
                    <p className="text-sm font-bold text-[hsl(var(--foreground))]">Report Ready!</p>
                    <p className="text-xs text-[hsl(var(--muted-foreground))]">{reportTypes.find(r => r.id === selectedReport)?.name} · {format} · {dateFrom} to {dateTo}</p>
                  </div>
                </div>
                <div className="flex gap-2">
                  ...
                  <button onClick={() => setGenerated(false)} className="px-4 py-2.5 rounded-xl text-xs font-semibold border border-[hsl(var(--border))] text-[hsl(var(--muted-foreground))] cursor-pointer whitespace-nowrap">
                    New Report
                  </button>
                </div>
              </div>
