...
- **Broken migrations can't go live:** `apply-migration.sh` applies a migration, records it, and runs `ops/wireless/db-checks.sql` in one transaction, committing only if the checks pass. The checks run `plpgsql_check` over every wireless PL/pgSQL function and a smoke test as one real user of each role. This was verified by reapplying the 2026-09-10 bell bug to production: it was rejected with 7 problems and nothing changed. That bug (a bare `id` in `get_my_notifications()`) broke the notification bell for all non-admin staff from 2026-09-10 to 2026-09-26. The monitor also runs these checks every 10 minutes and alerts on any database error that repeats.
- **Releases:** `ops/wireless/release.sh` only tags a release when `development` is pushed, every migration is applied in production, and the production DB checks pass. The production deploy workflow re-runs the CI suite and deploys only if it passes.
...
- **CI doesn't enforce typecheck or lint yet.** `.github/workflows/ci.yml` runs the role audit, the database trigger/policy tests, and the build on every push and PR, and the production deploy requires it. Typecheck isn't included because the codebase has about 278 pre-existing TypeScript errors (mostly `src/pages/whatsapp`). 
