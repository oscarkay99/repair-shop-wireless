...
- Read access to customers, invoices/items, sales/items, parts, and the technician roster is defined as permission-backed at RLS level in `20260918000000_fail_closed_role_authorization.sql`. Hiding a nav item is not treated as an authorization boundary. This statement becomes true for an environment only after that migration is applied.
...
- **The 2026-09-18 fail-closed RLS migration is not yet applied to production.** The frontend guards are live, but this workstation's Supabase CLI session is unauthorized and CI has no database credential. Until an authorized operator applies `20260918000000_fail_closed_role_authorization.sql`, active users may still query several hidden read-only tables directly through PostgREST.
